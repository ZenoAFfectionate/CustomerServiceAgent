# -*- coding: utf-8 -*-
"""rag/api 集成测试：FastAPI 接口可用性（文档管理/检索/问答/健康检查/运维看板/错误处理）。

这是全项目"纵向"覆盖面最广的测试文件：一次 HTTP 请求会依次经过
`knowledge_base/`（导入）→ `indexing/`（写入索引）→ `retrieval/`（检索融合精排）→
`generation/`（生成回答）→ `observability/`（监控落盘），是名副其实的全链路组合测试。
"""
import json
...
class TestDocumentAPI:
    ...
    def test_upload_oversized_file_returns_413(self, client, monkeypatch):
        """回归测试：修复审查报告 L8——上传超限文件应被拒绝（413），且现改为
        分块读取、超限即中止，不再先把整个文件读入内存才校验大小。"""
        from rag.config import RAG_CONFIG

        monkeypatch.setitem(RAG_CONFIG, "upload_max_size_mb", 1)
        oversized = b"x" * (2 * 1024 * 1024)  # 2MB，超过 1MB 限制
        resp = client.post(
            "/api/documents/upload",
            files={"file": ("big.txt", oversized, "text/plain")},
        )
        assert resp.status_code == 413
        assert resp.json()["error_code"] == "RAG_PAYLOAD_TOO_LARGE"
    ...
    def test_document_history_endpoint(self, client):
        """组合测试：/api/documents/{doc_id}/history 依赖 knowledge_base/versioning.py
        的版本记录，验证 API 层与知识库管理层正确对接。"""
        upload_resp = client.post(
            "/api/documents/ingest_blocks",
            json={"filename": "v.json", "blocks": [{"text": "版本历史测试内容"}]},
        )
        doc_id = upload_resp.json()["doc_id"]
        resp = client.get(f"/api/documents/{doc_id}/history")
        assert resp.status_code == 200
        history = resp.json()
        assert len(history) == 1
        assert history[0]["num_chunks"] == 1
...
class TestDashboardAPI:
    """组合测试：/api/dashboard 依赖 observability/ 全套（monitoring + alerting）与
    retrieval_selection/indexing 的后端配置汇总，验证多次请求后指标正确累积。"""

    def test_dashboard_returns_metrics_backends_and_corpus_stats(self, client):
        client.post("/api/documents/ingest_blocks", json={"filename": "x.json", "blocks": [{"text": "内容"}]})
        client.post("/api/retrieve", json={"query": "内容", "top_k": 3})
        client.post("/api/chat", json={"query": "内容"})

        resp = client.get("/api/dashboard")
        assert resp.status_code == 200
        data = resp.json()
        assert data["metrics"]["retrieval"]["count"] >= 1
        assert data["metrics"]["answer"]["count"] >= 1
        assert "vector_backend" in data["backends"]
        assert data["corpus_stats"]["num_documents"] == 1
