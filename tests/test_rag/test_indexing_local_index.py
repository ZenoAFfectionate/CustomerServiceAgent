# -*- coding: utf-8 -*-
"""rag/indexing/local_index.py 单元测试（本地降级实现：LocalVectorStore + LocalKeywordStore）。"""
import os
...
def _make_vec_block(gid, doc_id, text, embedding):
    return DocBlock(text=text, global_chunk_idx=gid, doc_id=doc_id, embedding=embedding)


def _make_kw_block(gid, doc_id, title, text):
...
class TestLocalVectorStore:
    def test_upsert_and_count(self, tmp_path):
        ...
        blocks = [_make_vec_block(0, "doc1", "文本A", [1.0, 0.0, 0.0]),
                  _make_vec_block(1, "doc1", "文本B", [0.0, 1.0, 0.0])]
    ...
    def test_upsert_is_idempotent_on_same_id(self, tmp_path):
        store = LocalVectorStore(path=str(tmp_path / "vs.json"))
        store.upsert([_make_vec_block(0, "doc1", "版本1", [1.0, 0.0])])
        store.upsert([_make_vec_block(0, "doc1", "版本2", [0.0, 1.0])])
        assert store.count() == 1

    def test_search_returns_most_similar_first(self, tmp_path):
        store = LocalVectorStore(path=str(tmp_path / "vs.json"))
        store.upsert([
            _make_vec_block(0, "doc1", "完全相同方向", [1.0, 0.0, 0.0]),
            _make_vec_block(1, "doc1", "正交方向", [0.0, 1.0, 0.0]),
            _make_vec_block(2, "doc1", "反方向", [-1.0, 0.0, 0.0]),
        ])
    ...
    def test_search_respects_top_k(self, tmp_path):
        store = LocalVectorStore(path=str(tmp_path / "vs.json"))
        store.upsert([_make_vec_block(i, "doc1", f"文本{i}", [float(i), 1.0]) for i in range(5)])
    ...
    def test_delete_by_doc_id(self, tmp_path):
        store = LocalVectorStore(path=str(tmp_path / "vs.json"))
        store.upsert([
            _make_vec_block(0, "docA", "A的块", [1.0, 0.0]),
            _make_vec_block(1, "docB", "B的块", [0.0, 1.0]),
        ])
...
class TestLocalKeywordStore:
    def test_upsert_and_count(self, tmp_path):
        ...
        store.upsert([
            _make_kw_block(0, "doc1", "广告限流", "账户存在异常投放行为时会被限流"),
            _make_kw_block(1, "doc1", "退款规则", "退款需在7天内申请"),
        ])
        assert store.count() == 2
    ...
    def test_search_returns_relevant_result_first(self, tmp_path):
        store = LocalKeywordStore(path=str(tmp_path / "ks.json"))
        store.upsert([
            _make_kw_block(0, "doc1", "广告限流规则", "千川广告投放出现异常会被限流处理"),
            _make_kw_block(1, "doc1", "退款政策", "商品签收后七天内可申请无理由退款"),
            _make_kw_block(2, "doc1", "物流查询", "可以在订单详情页查看物流轨迹信息"),
        ])
    ...
    def test_search_empty_query_returns_empty(self, tmp_path):
        store = LocalKeywordStore(path=str(tmp_path / "ks.json"))
        store.upsert([_make_kw_block(0, "doc1", "标题", "内容")])
    ...
    def test_search_respects_top_k(self, tmp_path):
        store = LocalKeywordStore(path=str(tmp_path / "ks.json"))
        store.upsert([_make_kw_block(i, "doc1", f"标题{i}", f"广告限流相关内容第{i}条") for i in range(5)])
    ...
    def test_delete_by_doc_id(self, tmp_path):
        store = LocalKeywordStore(path=str(tmp_path / "ks.json"))
        store.upsert([
            _make_kw_block(0, "docA", "A标题", "A的内容"),
            _make_kw_block(1, "docB", "B标题", "B的内容"),
        ])
        removed = store.delete_by_doc_id("docA")
        assert removed == 1
        assert store.count() == 1
    ...
    def test_persistence_across_instances(self, tmp_path):
        ...
        store1.upsert([_make_kw_block(0, "docA", "持久化", "持久化测试内容")])
    ...
    def test_unrelated_query_scores_low_or_filtered(self, tmp_path):
        store = LocalKeywordStore(path=str(tmp_path / "ks.json"))
        store.upsert([_make_kw_block(0, "doc1", "退款规则", "退款需在签收后七天内申请")])
