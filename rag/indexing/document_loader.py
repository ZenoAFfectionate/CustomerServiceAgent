# -*- coding: utf-8 -*-
"""文档解析（原 loader.py，重命名以对齐新的模块命名规范）：将上传文件解析为纯文本。
...
def _parse_html(raw: bytes, **_) -> str:
    ...
    # 与 process/src/html_utils.py 保持一致，使用标准库自带的 "html.parser"，
    # 避免引入 lxml 这个额外的系统级依赖（无需 C 扩展编译，开箱即用）。
...
def parse_document(filename: str, raw: bytes):
    ...
    Returns:
        ...
        - 其他格式：返回 str（纯文本，交给 chunking 分块）
    """
