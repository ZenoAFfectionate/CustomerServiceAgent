...
数据量增大后建议切换 `RAG_VECTOR_BACKEND=milvus`）。

线程安全性：本类由 `vector_store.get_vector_store()` 以**进程级单例**持有
（单实例、共享同一把 `threading.Lock`），因此同进程内的并发读写是安全的；
但仍是单文件 JSON 存储，不具备多进程/多副本部署下的并发安全性。
